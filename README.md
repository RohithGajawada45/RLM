...
On top of the base algorithm, this project adds a full product layer: multi-document upload and routing, cross-document fan-out and synthesis, a three-layer caching system, and a single-page web UI.
...
- 🎨 **Single-page frontend** — drag-and-drop upload, a live document library, a query box, and a response panel showing routing decisions, confidence bars, per-document findings, and token cost — no build step, plain HTML/CSS/JS.
...
│   ├── style.css              Design tokens, layout, components (1.4k lines)
│   └── app.js                  Upload/dropzone, query form, response rendering, token bar, markdown rendering
...
**1. Upload documents** — drag PDF/DOCX/TXT/MD files onto the dropzone. Each file triggers one LM call to generate its routing description; this typically takes a few seconds and only happens once per file (descriptions are reused across restarts).

**2. Ask a question** — the router automatically picks the mode:
...
**3. Read the response** — the panel shows which document(s) were used and why, a confidence bar, per-document findings for fan-out queries, a token-cost breakdown by stage, and route/RLM/total timing.
...
The frontend (`static/style.css`) uses IBM Plex-adjacent typography (Plus Jakarta Sans + JetBrains Mono) on a warm, cream-toned light palette — deliberately avoiding the generic "dark mode SaaS dashboard" look. Markdown answers are rendered client-side with `marked.js`.
